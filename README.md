...
**HexStrike AI** - это сервер MCP (Model Context Protocol), предназначенный для того, чтобы действовать как мост между моделями ИИ (такими как модели ChatGPT, Claude или даже локальные модели LLaMA) и реальными инструментами безопасности.

С его помощью ИИ может напрямую:
- **Выполнить разведку**
- **Запуск веб- и сетевое сканирование**
- **Обработка двоичной эксплуатации и обратного инжиниринга**
- **Проведение судебно-медицинской экспертизы и расследований OSINT**
- **Даже генерировать рабочие эксплойты на основе выводов**
...
```
Перейдите во вкладку настройки справа вверху
...
Зайдите в раздел MCP & Integrations и создайте новый MCP Server
...
Откроется json файл и вставьте туда следующее сообщение заменив путь к файлам в директории куда установили hexstrike. Также поменяйте адрес на адрес своей VM.
